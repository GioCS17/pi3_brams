PROGRAM MainGrib
  ...
  INTEGER, DIMENSION(2000) :: LEVPR
  REAL, DIMENSION(2000) :: ylev

  REAL :: WPLON, SPLAT, spcnprx, spcnpry

  CHARACTER(LEN=128) :: fprefix
  CHARACTER(LEN=128) :: ifnpress
  CHARACTER(LEN=200) :: prep_in_path
  ...
  CHARACTER(LEN=200), DIMENSION(maxfiles) :: fnames
  INTEGER :: nfile
  ...
  !-----------------------------------------------------------------
  call get_command_argument(1, prep_in_path)
  open(1,file=prep_in_path,status='old')
  read(1,data_prep)
  close(1)
...
SUBROUTINE MakeDP(nfile,fname,nx,ny,nz,nvar,splat,wplon,spcnprx,spcnpry, &
  ...
  CHARACTER(LEN=100)  :: fnamesout
  CHARACTER(LEN=100)  :: original_path
  ...
  fnamesout=fname(:LEN(fname) - 4)//'.txt'
  original_path=fname(:LEN(fname) - 51)
  command = '$GRIB2DP_PATH/grbconv -i '//fname//' -o '//fnamesout//' -t cona'
  CALL system(command)
  ...
  nc=INDEX(ifnpress,' ')-1
  PRINT *,original_path,ifnpress(1:nc),iyear,'-',imonth,'-',idate,'-',ihour*100
  ...
  OPEN(11,file=TRIM(ADJUSTL(original_path))//locfn,status='unknown',form='formatted')
  REWIND (11)
...
SUBROUTINE GRIB_filelist(fnames,nfile, fprefix)
  ...
  ! ************** DECLARATIONS *****************
  CHARACTER(LEN=256) :: file, command
  INTEGER :: iun, nf
